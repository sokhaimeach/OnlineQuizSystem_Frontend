...
export function DataTable<TData, TValue>({
  ...
  const firstRowNumber = data.length === 0 ? 0 : resolvedPagination.pageIndex * resolvedPagination.pageSize + 1
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border px-4 py-3">
          <p className="text-xs text-muted-foreground">
            {enableRowSelection && table.getFilteredSelectedRowModel().rows.length > 0
              ...
              : ""}
            {firstRowNumber}–{lastRowNumber} of {rowCount ?? table.getFilteredRowModel().rows.length}
          </p>
          <div className="flex items-center gap-2">
            <span className="text-xs text-muted-foreground">
              Page {resolvedPagination.pageIndex + 1} of {Math.max(table.getPageCount(), 1)}
            </span>
            <Button
              variant="outline"
              size="icon-sm"
