...
const allQuestions: Question[] = [
  ...
  {
    id: 5,
    text: "Solve the system: 2x + y = 5, x - y = 1",
    type: "Short Answer",
    difficulty: "hard",
    subject: "Mathematics",
  ...
  {
    id: 8,
    text: "Derive the formula for kinetic energy.",
    type: "Short Answer",
    difficulty: "hard",
    subject: "Physics",
    topic: "Energy",
...
const columns: ColumnDef<Question>[] = [
  {
    ...
    header: "Question",
    cell: ({ row }) => (
      <p className="max-w-xs truncate font-medium">{row.original.text}</p>
    ),
  },
  {
    accessorKey: "type",
    ...
    cell: ({ row }) => {
      const TypeIcon =
        typeIcon[row.original.type as keyof typeof typeIcon] ?? FileQuestion;
      return (
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div>
        <p className="text-xs font-medium">{row.original.subject}</p>
  ...
  },
  {
    ...
    header: "Difficulty",
    cell: ({ row }) => (
      <StatusBadge variant={difficultyVariant[row.original.difficulty]}>
        {row.original.difficulty[0].toUpperCase() +
          row.original.difficulty.slice(1)}
      </StatusBadge>
    ),
  },
  {
    ...
    cell: ({ row }) => (
      <span className="text-xs text-muted-foreground">
        {row.original.usedIn ? `${row.original.usedIn} quizzes` : "Unused"}
      </span>
    ),
  },
  {
    ...
    cell: () => (
      <div className="flex gap-1">
        <Button size="icon-sm" variant="ghost" aria-label="Edit question">
          <Edit />
        </Button>
        <Button
          size="icon-sm"
        ...
        <div className="flex flex-wrap gap-1">
          {types.map((type) => (
            <Button
              ...
              className={
                typeFilter === type ? "border-primary text-primary" : ""
              }
