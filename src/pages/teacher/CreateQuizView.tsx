...
export function CreateQuizView({ onNavigate }: CreateQuizViewProps) {
  ...
  const selectedSubject = subjects.find(
    (subject: any) => subject.id === quiz.subject_id,
  );

  const updateQuiz = <Key extends keyof CreateQuiz>(
    key: Key,
    value: CreateQuiz[Key],
  ) => {
  ...
  const updateQuestion = (index: number, question: QuestionWithOptions) => {
    updateQuiz(
      ...
      quiz.questions.map((item, questionIndex) =>
        questionIndex === index ? question : item,
      ),
    );
  };
  ...
  const validateStep = (stepIndex: number) => {
    ...
    if (stepIndex === 2) {
      for (let index = 0; index < quiz.questions.length; index += 1) {
        ...
        if (question.score < 1)
          return `${label} must be worth at least 1 point.`;
  ...
  const publishQuiz = async () => {
    ...
    try {
      ...
      setError(
        "The quiz could not be published. Please check your connection and try again.",
      );
    }
  };
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-2">
              ...
              <SearchableSelectInput
                options={subjects
                  .filter((subject): subject is Subject & { id: string } =>
                    Boolean(subject.id),
                  )
                  .map((subject) => ({
                    value: subject.id,
                ...
                disabled={isLoading}
                placeholder={
                  isLoading ? "Loading subjects…" : "Write to search subjects"
                }
                emptyMessage="No similar subjects found."
              />
              <p className="text-xs text-muted-foreground">
                Optional. Search by name; the selected subject ID is stored in
                the payload.
              </p>
            </div>
            <div className="flex flex-col gap-2">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <div className="flex flex-col gap-2">
                ...
                <Input
                  ...
                  onChange={(event) =>
                    updateQuiz("passing_score", Number(event.target.value) || 0)
                  }
                />
              </div>
            </div>
            ...
            {(
              [
                [
                  ...
                  "Show the score as soon as a student submits.",
                ],
                [
                  "show_correct_answers",
                  "Show correct answers",
                  "Reveal correct answers with the result.",
                ],
                [
                  "randomize_questions",
                  "Randomize questions",
                  "Use a different question order for each attempt.",
                ],
              ] as const
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              {[
                ...
                {
                  label: "Subject",
                  value: selectedSubject?.subject_name ?? "No subject",
                },
                {
                  label: "Time Limit",
                ...
                <div
                  key={item.label}
                  className="flex flex-col gap-0.5 rounded-lg bg-muted/30 p-3"
                >
            ...
            <div className="flex flex-col gap-2">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  This choice is submitted as the boolean <code>is_public</code>{" "}
                  value.
                </p>
              </div>
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
