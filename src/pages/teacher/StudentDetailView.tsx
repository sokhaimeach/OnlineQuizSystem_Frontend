...
import { useGetStudentAttemptsHistories, useGetStudentById } from "@/hooks/api/useStudent"
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        ...
        <PageHeader title={fullName} description={`Student ID: ${id}`} icon={UserRound} />
      </div>
      ...
      {studentQuery.isLoading ? (
        ...
        <QueryError
          message={studentQuery.error instanceof Error ? studentQuery.error.message : "Student details were not found."}
      ...
      <section className="space-y-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Quiz submissions and scores for this student.</p>
        </div>
        {attemptsQuery.isError ? (
          <QueryError
            message={attemptsQuery.error instanceof Error ? attemptsQuery.error.message : "Attempt history could not be loaded."}
