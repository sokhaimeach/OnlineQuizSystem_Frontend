...
export const useGetAssignmentByClassId = (classId: string) => {
    return useQuery({
        queryKey: ['assignments', classId],
        queryFn: async () => {
            const response = await getAssignmentByClassId(classId)
...
export const useUpdateAssignment = () => {
    const queryClient = useQueryClient()
    return useMutation({
        mutationFn: ({ assignmentId, payload }: { assignmentId: string; payload: Partial<CreateAssignment> }) =>
